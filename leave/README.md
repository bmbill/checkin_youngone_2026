...
所以實際上是「每 15 分鐘最多一次」，**整個營隊只要有一個人的 app 去抄就好**。
去報到名單抄是慢的（要打 Apps Script），這件事只有「過期後第一個檢查到的人」會做。

其他手機不會重複去抄，但會**補抓一次資料庫裡那份新名單**——這一步很快，只讀資料庫。
少了它就只對了一半：資料庫是新的，可是一直開著頁面的人畫面上還是舊名單。

同步要好幾秒而輪詢是 8 秒一次，所以有一道閘門擋著，前一次還沒回來不會再發第二次。
另外，名單在背景換新時**不會重繪填單畫面**——關懷員可能正打到一半，重繪會把字弄丟。
...
| 輪詢 | 頁面開著、訂閱斷線也能用 | 見下面的節奏表 |
| 鎖屏推播 | 頁面關著、手機鎖著 | Web Push |

輪詢的節奏會自己調整。一次只有幾百 bytes，流量不是問題 ——
耗電才是：手機的行動網路模組送完資料後會維持喚醒十幾秒才降回待機，
固定 8 秒一次等於讓它整晚睡不著。

| 狀況 | 節奏 | 每小時 |
|---|---|---|
| 螢幕關掉、切到別的 app | 完全不送 | 0 |
| 即時訂閱通了 | 30 秒（訂閱負責即時，輪詢只是保險） | 120 |
| 剛剛有人操作，或有單子在動 | 8 秒 | 450 |
| 放著沒動超過 3 分鐘 | 25 秒 | 144 |

手指一碰螢幕、切回這一頁、或偵測到有單子變動，都會立刻回到 8 秒。
所以「有人在等簽核結果」的那幾分鐘一定是快的，其餘時間讓無線電睡覺。
