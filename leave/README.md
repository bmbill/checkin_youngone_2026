...
* 有名單但超過 6 小時 → 舊名單先用，背景靜靜換新，不打斷任何人
* 名單還新 → 完全不動，登入不會因此變慢
...
```sql
update lv_students set updated_at = now() - interval '10 hours';
```
...
**沒有排程，是「有人開 app 才檢查」。** 每次有關懷員登入時：

| 資料庫裡的名單 | 行為 |
|---|---|
| 空的 | 擋著畫面抓一次 |
| 超過 6 小時 | 背景更新（那位關懷員不會感覺到） |
| 不到 6 小時 | 不動 |

所以實際上是「每 6 小時最多一次」，由那段時間內第一個開 app 的人觸發。
沒有人開 app 就不會更新 —— 這是刻意的，避免整晚一直去打報到系統的 Apps Script。
